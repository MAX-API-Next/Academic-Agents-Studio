...
- 🖼️ **科研绘图**：通过独立的 GPT Image 2 绘图面板生成图形摘要、论文配图、概念示意图和演示素材
...
| | 会话与文件分区      | 配置登录认证后，上传文件和多数生成结果按用户名分区 |
...
Academic Agents Studio 提供独立的“绘图功能区”，适合生成论文图形摘要、研究流程示意图、概念结构图、科研海报素材和演示文稿配图。该功能通过 OpenAI 兼容的 Images API 调用 `gpt-image-2`，不会占用普通聊天回调，可在后台持续生成。

### 使用方法

1. 展开页面中的“绘图功能区”。
2. 输入图片描述，建议同时说明主体、布局、视觉风格、文字语言和使用场景。
3. 选择分辨率、质量和输出格式，然后点击“生成图片（GPT Image 2）”。
4. 生成期间页面会显示加载状态；如不再需要，可点击任务右侧的“停止”。
5. 完成后可直接预览并下载原图。

支持的参数：

- 分辨率：`auto`、`1024x1024`、`1536x1024`、`1024x1536`、`2048x2048`、`2048x1152`、`3840x2160`、`2160x3840`
...
Docker 或服务器部署可以使用 `GPT_ACADEMIC_IMAGE_API_URL`、`GPT_ACADEMIC_IMAGE_MODEL` 和 `GPT_ACADEMIC_IMAGE_TIMEOUT_SECONDS` 覆盖这些配置。绘图默认沿用 `API_KEY`（或 `GPT_ACADEMIC_API_KEY`）；对应令牌必须拥有目标图片模型的访问权限，否则接口会返回 `HTTP 403`。
