...
## 🎨 Создание научных иллюстраций (GPT Image 2)

Панель **Рисование** позволяет создавать графические аннотации, иллюстрации для статей, схемы исследовательских процессов, концептуальные рисунки, а также материалы для постеров и презентаций через совместимый с OpenAI Images API.

- Введите подробное описание, затем выберите разрешение, качество и формат результата.
- Поддерживаются разрешения от `1024x1024` до `3840x2160`, а также форматы `png`, `jpeg` и `webp`.
- Генерация выполняется как фоновая задача с индикатором загрузки; её можно остановить кнопкой рядом с сообщением о состоянии.
- Готовое изображение можно просмотреть и скачать в исходном формате.
- Настройте `IMAGE_API_URL`, `IMAGE_MODEL` и `IMAGE_TIMEOUT_SECONDS` в `config.py`. API-токен должен иметь доступ к выбранной модели изображений.
