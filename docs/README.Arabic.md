...
## 🎨 إنشاء الرسوم العلمية (GPT Image 2)

تتيح لوحة **الرسم** إنشاء الملخصات الرسومية، ورسوم الأوراق العلمية، ومخططات سير البحث، والرسوم المفاهيمية، ومواد الملصقات والعروض التقديمية عبر واجهة Images API متوافقة مع OpenAI.

- أدخل وصفًا تفصيليًا، ثم اختر الدقة والجودة وصيغة الإخراج.
- تتراوح الدقات المدعومة من `1024x1024` إلى `3840x2160`، والصيغ المتاحة هي `png` و`jpeg` و`webp`.
- تعمل عملية الإنشاء كمهمة في الخلفية مع مؤشر تحميل، ويمكن إيقافها من الزر بجانب رسالة الحالة.
- يمكن معاينة الصورة المكتملة وتنزيل الملف الأصلي.
- اضبط `IMAGE_API_URL` و`IMAGE_MODEL` و`IMAGE_TIMEOUT_SECONDS` في `config.py`، ويجب أن يملك رمز API صلاحية الوصول إلى نموذج الصور المحدد.
