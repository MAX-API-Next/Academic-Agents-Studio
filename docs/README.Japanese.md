...
## 🎨 科研・学術イラスト生成（GPT Image 2）

**描画機能エリア**では、OpenAI 互換の Images API を利用して、グラフィカルアブストラクト、論文用図版、研究フロー図、概念図、ポスターや発表資料向けの画像を生成できます。

- 詳細な説明を入力し、解像度、品質、出力形式を選択します。
- 対応解像度は `1024x1024` から `3840x2160` まで、形式は `png`、`jpeg`、`webp` です。
- 生成は読み込み表示付きのバックグラウンドタスクとして実行され、状態メッセージ横のボタンから停止できます。
- 完成した画像はプレビューし、元の形式でダウンロードできます。
- `config.py` の `IMAGE_API_URL`、`IMAGE_MODEL`、`IMAGE_TIMEOUT_SECONDS` を設定してください。API トークンには対象画像モデルへのアクセス権が必要です。
