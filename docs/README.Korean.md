...
## 🎨 과학·학술 이미지 생성 (GPT Image 2)

**그리기 기능 영역**에서는 OpenAI 호환 Images API를 사용하여 그래픽 초록, 논문 삽화, 연구 흐름도, 개념도, 포스터 및 발표 자료용 이미지를 생성할 수 있습니다.

- 상세한 설명을 입력한 뒤 해상도, 품질, 출력 형식을 선택합니다.
- 지원 해상도는 `1024x1024`부터 `3840x2160`까지이며, 형식은 `png`, `jpeg`, `webp`입니다.
- 생성 작업은 로딩 표시가 있는 백그라운드 작업으로 실행되며 상태 메시지 옆의 버튼으로 중지할 수 있습니다.
- 완료된 이미지는 미리 보고 원본 형식으로 다운로드할 수 있습니다.
- `config.py`에서 `IMAGE_API_URL`, `IMAGE_MODEL`, `IMAGE_TIMEOUT_SECONDS`를 설정하십시오. API 토큰에는 선택한 이미지 모델의 사용 권한이 필요합니다.
